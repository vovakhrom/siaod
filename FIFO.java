...
//главная подоочередь . Массивы с ссылками на первые элементы , последние элементы и размер подочередей adr[0] - номер подочереди . adr[1] - номер элемента в подочереди
//список
class El<E> {
	private E val;
	private El next;
